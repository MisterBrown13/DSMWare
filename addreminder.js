	...
	//alert($task_id);
	  ...
	  $.getJSON($root+"me/lists",function(data2)
		...
		while(data2[$j])
		    ...
		    $.getJSON($root+"me/reminders",function(data3)
		    {
			if(data3){
				$ii=0;
				while(data3[$ii]){
				    $node2 = document.createElement("p");
				    $textnode2=document.createTextNode(data3[$ii].date);
				    $node2.appendChild($textnode2);
				    document.getElementById("reminder").appendChild($node2);
				    $ii++;
				}
...
dataString = $("#reminder_form").serialize();
            //alert("ok");
            //get the form data using another method
            var date = $("input#date").val();
	    var time = $("input#heure").val();
	    $task_id=$urlParams.id;
            dataString = 'task_id=' + $task_id +'&date=' + date + "T" + time +"Z";
            alert(dataString);
